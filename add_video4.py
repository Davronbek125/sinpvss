...
def add_fourth_video(filename):
    ...
    if '</iframe>\n            </div>' in content:
        ...
        with open(filename, 'w', encoding='utf-8') as f:
            f.write(content)
        print(f"Updated {filename} with 4th video")
    else:
        print(f"Could not find video div in {filename}")

add_fourth_video('sinov-laboratoriyasi.html')
add_fourth_video('sinov-laboratoriyasi_ru.html')
